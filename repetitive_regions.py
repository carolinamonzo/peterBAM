...
# Getting reads mapping to repetitive regions, classifying per type
...
def parseArguments():
    ...
    parser = argparse.ArgumentParser(description = "Study types of repetitive regions from bam files")
...
def cmd_get_repetitive(config, fof):
    """
    Function to get the merged bam files per chromosome with all individuals
    Input: config dictionary with path to files
    Output: cmd file
    """
    cmd_sh = datetime.datetime.now().strftime("cmd_get_repetitive_%Y%m%d_%H-%M-%S.sh")

    with open(config["paths"]["cmd_files"] + cmd_sh, "a") as cmd_file:
        for bam in fof:
            bam_name = bam.split("/")[-1].split(".")[0]
            cmd_str = "bedtools intersect -wb -a {} -b {}/new-hg19_repetitive-clean.bed -bed > {}/{}_RPregions.bed".format(bam, config["paths"]["bed"], config["paths"]["offtarget"], bam_name)
...
def run_parallel(config, cmd_sh):
    ...
    log_str = datetime.datetime.now().strftime("get_offtarget_%Y%m%d_%H-%M-%S.log")

    cmd = "parallel --joblog {}{} -j 18 :::: {}{}".format(config["paths"]["offtarget"], log_str, config["paths"]["cmd_files"], cmd_sh)
...
def write_output_fof(config):
    ...
    fof = datetime.datetime.now().strftime("RPregions_%Y%m%d_%H-%M-%S.fof")
    # Create fof file

    cmd_fof = 'find {} -name "*_RPregions.bed" > {}{}'.format(config["paths"]["offtarget"], config["paths"]["fof_files"], fof)
