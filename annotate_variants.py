...
def parseArguments():
    ...
    parser = argparse.ArgumentParser(description = "Annotate variants")
...
def read_input_fof(config):
    ...
    for file in glob.glob(config["paths"]["fof_files"] + "merged_vcf_*.fof"):
...
def cmd_annotate_vcf(config, fof):
    ...
    with open(config["paths"]["cmd_files"] + cmd_sh, "a") as cmd_file:
        for vcf in fof:
            ...
            cmd_str = "vep -i {} --format vcf --vcf -o {}{}_vep.vcf --cache_version 92 --dir_cache /nfs/qnapugdg8tb3a/nfs_databases/vep/vep --assembly GRCh37 --offline --force_overwrite --everything --sift s --polyphen s; bgzip -c {}{}_vep.vcf > {}{}_vep.vcf.gz; tabix -p vcf {}{}_vep.vcf.gz".format(vcf, config["paths"]["variant_annotation"], vcf_name, config["paths"]["variant_annotation"], vcf_name, config["paths"]["variant_annotation"], vcf_name, config["paths"]["variant_annotation"], vcf_name)
...
def write_output_fof(config):
    ...
    fof = datetime.datetime.now().strftime("variant_annotation_%Y%m%d_%H-%M-%S.fof")

    cmd_fof = 'find {} -name "17NR*-merged-sorted_vep.vcf.gz" > {}{}'.format(config["paths"]["variant_annotation"], config["paths"]["fof_files"], fof)
