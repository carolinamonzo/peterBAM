...
# Utils for peterBAM.py - pipeline for deepseq analysis
...
def parseArguments():
    ...
    parser = argparse.ArgumentParser(description = "Pipeline for DeepSeq analysis \
 'peterBAM'" + '/n' + '/n' + "python3.5 utils.py -fq fastq_path -b bed_file -p project_path")
...
def load_config(args):
    ...
    """
    with open("/nfs/production3b/cmc_projects_3b/peterBAM/config.json", "r") as jconfig:
...
def create_directories(config, logger):
    ...
    for dire in ['plots', 'coverage', 'fastq', 'mapping', 'mapping_QC',
    ...
    logger.info("Creating symlink to original fastq files using 'symlink_fastq.sh script from peterBAM'\n")
