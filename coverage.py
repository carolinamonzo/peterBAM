...
# Script to obtain coverage per base of ROI
...
def cmd_coverage(config, fof):
    ...
    cmd_sh = datetime.datetime.now().strftime("cmd_coverage_%Y%m%d_%H-%M-%S.sh")
...
def write_output_fof(config):
    ...
    fof = datetime.datetime.now().strftime("coverage_ROI_%Y%m%d_%H-%M-%S.fof")

    cmd_fof = 'find {} -name "*.pbcov" > {}{}'.format(config["paths"]["coverage"], config["paths"]["fof_files"], fof)
