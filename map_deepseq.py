...
def read_input_fof(config):
    ...
    for file in glob.glob(config["paths"]["fof_files"] + "trimmed_fastq_files_*.fof"):
...
def merged_fastq_dataframe(config, fastq_files):
    ...
    rex = re.compile(r"{}(?P<sample>17NR\d+)_R(?P<read>\d+)-trimmed.fastq.gz".format(config["paths"]["fastq_trimmed"]))
